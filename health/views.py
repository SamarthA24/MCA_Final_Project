...
def get_risk_precautions(prediction):
    """
    Generate risk factors and precautions using Google Gemini AI.
    """
    model = genai.GenerativeModel("gemini-1.5-pro-latest")

    prompt = f"""
    You are a medical expert providing heart disease guidance.
    A patient has been classified with a **{prediction}** risk level for heart disease.

    **Provide:**
    - **Top 5 key risk factors** contributing to this condition.
    - **5 important precautions** they should take.
    ...
    **Precautions:**
    ...
    try:
        ...
        print("🔹 Gemini API Response:", response.text if response else "No response received")

        if response and response.text:
            return response.text.replace("\n", "<br>")  # Ensures proper line breaks in HTML
        else:
            return "⚠ Unable to fetch recommendations from AI."
    except Exception as e:
        print(f"❌ Error fetching recommendations: {str(e)}")  # Debugging
...
def predict_desease(request, pred, accuracy):
    ...
    # Fetch risk factors & precautions using Gemini AI
    risk_precautions = get_risk_precautions(pred)

    # Debugging: Print output before sending to template
    print("🔹 Final Risk Precautions Output:", risk_precautions)
